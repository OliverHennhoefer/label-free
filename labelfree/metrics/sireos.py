...
def sireos(
    ...
    if scores_max == scores_min:
        scores_normalized = np.ones_like(scores) * 0.5
    else:
    ...
    for i in range(n_samples):
        ...
        # Compute exponential kernel similarities (excluding self-distance)
        similarities = np.exp(-(point_distances**2) / (2 * threshold**2))

        # Exclude self-similarity (set diagonal to 0)
        similarities[i] = 0.0

        # Compute mean similarity (excluding self)
        mean_similarity = np.sum(similarities) / (n_samples - 1)

        # Weight by normalized score
        total_score += mean_similarity * scores_normalized[i]

    # Normalize by number of samples
    sireos_score = total_score / n_samples
