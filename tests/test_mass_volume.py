...
class TestMassVolume:
    ...
    def test_basic_functionality(self):
        ...
        assert np.all(
            result["volume"] >= 0
        )  # Volume can be > 1 due to volume_support scaling
    ...
    def test_perfect_detector(self):
        ...
        ), f"Volume should generally decrease with mass, got {decreasing_ratio:.2f} decreasing"
    ...
    def test_volume_support_warning(self):
        ...
        rng = np.random.default_rng(42)
        X = rng.standard_normal((100, 2)) * 100  # Large scale data
        scores = rng.standard_normal(100)
    ...
    def test_perfect_vs_random_comparison(self):
        ...
        random_result = mass_volume_auc(
            random_scores, X, n_thresholds=100, random_state=42
        )
    ...
    def test_identical_scores(self):
        ...
        assert (
            result["mass"][-1] == 1.0
        ), "With identical scores, final mass should be 1.0"
        ...
        assert (
            volume_increases <= len(result["volume"]) * 0.2
        ), "Volume should be mostly stable with identical scores"
    ...
    def test_single_point_data(self):
        ...
        result = mass_volume_auc(
            scores, X, n_thresholds=10, n_mc_samples=100, random_state=42
        )
    ...
    def test_degenerate_data_single_dimension(self):
        ...
        rng = np.random.default_rng(42)
        X = np.column_stack(
            [rng.standard_normal(100), np.ones(100)]
        )  # Second dim is constant
        scores = rng.standard_normal(100)
    ...
    def test_monte_carlo_accuracy(self):
        ...
        result = mass_volume_auc(
            scores, X, n_thresholds=50, n_mc_samples=10000, random_state=42
        )
    ...
    def test_axis_alpha_mass_alignment(self):
        ...
        assert (
            max_diff <= 0.05
        ), f"Mass should align with axis_alpha, max diff: {max_diff:.4f}"
    ...
    def test_regression_golden_standard(self):
        ...
        expected_auc = 13.77  # Updated for new random generator
        assert (
            abs(result["auc"] - expected_auc) < 1.0
        ...
        assert (
            volume_increases <= len(result["volume"]) * 0.8
    ...
    def test_different_data_scales(self):
        ...
        result_small = mass_volume_auc(
            scores, X_small, n_thresholds=20, random_state=42
        )
        result_large = mass_volume_auc(
            scores, X_large, n_thresholds=20, random_state=42
        )

        # AUC should scale with data volume
        # Small data should have smaller AUC, large data should have larger AUC
        assert (
            result_small["auc"] < result_base["auc"] < result_large["auc"]
