...
def reference_mass_volume(alpha_min, alpha_max, volume, uniform_scores, anomaly_scores, alpha_count=1000):
    """
    ...
    mass = 0
    cpt = 0
    threshold = anomaly_scores[indices[-1]] if n_samples > 0 else 0  # Start from highest score
...
def reference_mass_volume_auc(alpha_min, alpha_max, volume, uniform_scores, anomaly_scores, alpha_count=1000):
    ...
    return {
        "mass": np.linspace(alpha_min, alpha_max, alpha_count),  # Simplified for comparison
        "volume": mv_scores,
        "auc": auc,
...
class TestMassVolumeReference:
    ...
    def test_debug_algorithm_differences(self):
        ...
        our_simple = mass_volume_auc(
            scores_simple, X_simple, alpha_min=0.8, alpha_max=0.95,
            n_thresholds=3, n_mc_samples=len(uniform_scores_simple),
            random_state=42, scoring_function=scoring_function
        )
        ...
        print(f"Simple - Our AUC: {our_simple['auc']:.6f}, Ref AUC: {ref_simple['auc']:.6f}")
        print(f"Simple - Relative error: {abs(our_simple['auc'] - ref_simple['auc']) / ref_simple['auc']:.8f}")
        
        # Test case 2: More realistic case
        print("\\n=== REALISTIC CASE ===")
        X_real = np.random.RandomState(42).randn(20, 2)  # 20 points, 2D
        scores_real = np.random.RandomState(42).randn(20)
        ...
        our_real = mass_volume_auc(
            scores_real, X_real, alpha_min=0.9, alpha_max=0.999,
            n_thresholds=10, n_mc_samples=len(uniform_scores_real),
            random_state=42, scoring_function=scoring_function_real
        )
        ...
        print(f"Realistic - Our AUC: {our_real['auc']:.6f}, Ref AUC: {ref_real['auc']:.6f}")
        ...
        simple_error = abs(our_simple['auc'] - ref_simple['auc']) / ref_simple['auc'] if ref_simple['auc'] != 0 else 0
        assert simple_error < 0.001, f"Simple case error: {simple_error:.6f}"
        
        # Realistic case should be much better than current 3-5%
        assert realistic_error < 0.02, f"Realistic case error too high: {realistic_error:.4f}"
    ...
    def test_basic_equivalence(self):
        ...
        our_result = mass_volume_auc(
            scores, X, alpha_min=alpha_min, alpha_max=alpha_max,
            n_thresholds=n_thresholds, n_mc_samples=n_mc_samples, 
            random_state=random_state, scoring_function=scoring_function
        )
        ...
        relative_error = abs(our_result["auc"] - ref_result["auc"]) / max(ref_result["auc"], 1e-12)
        ...
        np.testing.assert_allclose(our_result["axis_alpha"], ref_result["axis_alpha"], rtol=1e-10)
    
    def test_perfect_detector_equivalence(self):
        """Test equivalence with perfect anomaly detector."""
        X, y = make_blobs_with_anomalies(n_samples=300, n_anomalies=30, random_state=123)
    ...
    def test_random_detector_equivalence(self):
        ...
        X, y = make_blobs_with_anomalies(n_samples=400, n_anomalies=40, random_state=456)
    ...
    def test_volume_computation_consistency(self):
        ...
        X = np.random.randn(100, 3) * 2 + 1  # Non-unit scale data
        scores = np.random.randn(100)
        
        # Our internal volume computation
        our_result = mass_volume_auc(scores, X, n_thresholds=10, n_mc_samples=500, random_state=789)
        
        # Generate identical uniform scores for comparison
        uniform_samples, uniform_scores = generate_consistent_uniform_scores(X, scores, 500, random_state=789)
        external_volume = compute_volume_support(X)
        
        # Reference with identical uniform scores
        ref_result = reference_mass_volume_auc(0.9, 0.999, external_volume, uniform_scores, scores, 10)
    ...
    def test_identical_scores_equivalence(self):
        ...
        scoring_function = create_scoring_function(uniform_scores)
        our_result = mass_volume_auc(
            scores, X, n_thresholds=n_thresholds, n_mc_samples=n_mc_samples, 
            random_state=random_state, scoring_function=scoring_function
        )
    ...
    def test_single_threshold_equivalence(self):
        ...
        X, y = make_blobs_with_anomalies(n_samples=100, n_anomalies=10, random_state=111)
        ...
        scoring_function = create_scoring_function(uniform_scores)
        our_result = mass_volume_auc(
            scores, X, alpha_min=0.95, alpha_max=0.99,
            n_thresholds=n_thresholds, n_mc_samples=n_mc_samples, 
            random_state=random_state, scoring_function=scoring_function
        )
    ...
    def test_large_dataset_equivalence(self):
        ...
        X, y = make_blobs_with_anomalies(n_samples=1000, n_anomalies=100, random_state=777)
