...
def objective(trial):
    ...
    sireos_score = labelfree.sireos(test_scores, x_test)

    trial.set_user_attr("roc_auc", roc_auc)
    trial.set_user_attr("pr_auc", pr_auc)
    trial.set_user_attr("sireos_score", sireos_score)
...
sireos_scores = np.array([t.user_attrs["sireos_score"] for t in completed_trials])

# Results
spearman_roc_sireos = spearmanr(roc_aucs, sireos_scores)[0]
spearman_pr_sireos = spearmanr(pr_aucs, sireos_scores)[0]

print("\n" + "=" * 50)
print("SIREOS Evaluation")
print("=" * 50)
print(f"SIREOS vs ROC-AUC correlation: {spearman_roc_sireos:.3f}")
print(f"SIREOS vs PR-AUC correlation:  {spearman_pr_sireos:.3f}")
