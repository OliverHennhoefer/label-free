...
roc_assessment = assess_proxy_quality(abs(spearman_roc_me), len(set(np.argsort(roc_aucs)[-10:]) & set(np.argsort(me_aucs)[-10:]))/10, roc_regret)
pr_assessment = assess_proxy_quality(abs(spearman_pr_me), len(set(np.argsort(pr_aucs)[-10:]) & set(np.argsort(me_aucs)[-10:]))/10, pr_regret)
