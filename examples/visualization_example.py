...
def plot_mass_volume_curve(scores, data, title="Mass-Volume Curve"):
    ...
    result = labelfree.mass_volume_curve(
        scores, data, n_thresholds=100, random_state=42
    )
    ...
    plt.plot(
        ...
        "b-",
        linewidth=2,
        label=f'MV Curve (AUC={result["auc"]:.3f})',
    )
...
def main():
    ...
    X_normal, _ = make_blobs(
        n_samples=500, centers=3, n_features=2, cluster_std=1.5, random_state=42
    )
    ...
    plot_excess_mass_curve(
        scores, volume_scores, "Excess-Mass Curve - Isolation Forest"
    )
